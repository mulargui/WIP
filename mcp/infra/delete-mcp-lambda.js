...
async function deleteFunctionUrl(functionName) {
  try {
    ...
    try {
      ...
      console.log("MCP Function URL public access permission removed successfully");
    } catch (error) {
      if (error.name !== "ResourceNotFoundException") {
        console.error("Error removing MCP function permission:", error);
      }
    }
    ...
    console.log("MCP Function URL deleted successfully");
  } catch (error) {
    if (error.name === "ResourceNotFoundException") {
      console.log("No MCP function URL found. Skipping deletion.");
    } else {
      throw error;
    }
  }
...
async function deleteLambda() {
  try {
    ...
    console.log(`MCP Lambda function ${FUNCTION_NAME} deleted successfully`);

    // Finally, delete the IAM role
    await deleteRole(ROLE_NAME);
  } catch (error) {
    console.error("Error deleting MCP Lambda function:", error);
  }
}
