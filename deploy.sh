...
cp $(pwd)/config.json $(pwd)/mcp
cp $(pwd)/config.json $(pwd)/mcp/src 
...
docker run --rm -w /src -v $(pwd)/mcp:/src node:22 \
	npm install @aws-sdk/client-lambda @aws-sdk/client-iam adm-zip
docker run --rm -w /src -v $(pwd)/mcp/src:/src node:22 \
	npm install @aws-sdk/client-bedrock-runtime @aws-sdk/client-rds mysql2
docker run --rm -w /src -v $(pwd)/mcp:/src \
	-e AWS_ACCESS_KEY_ID -e AWS_SECRET_ACCESS_KEY -e AWS_ACCOUNT_ID \
	-e AWS_REGION -e AWS_DEFAULT_REGION -e AWS_SESSION_TOKEN \
    node:22 node infra/deploy-mcp-lambda.js
